...
int main(int argc, char **argv) {
    ros::init(argc, argv, "pub1_add2_half_sec");  // 위의 ros.h 파일을 가져왔기 때문에 ros를 사용할 수 있다
    ...
    ros::Rate loop_rate(2); // 루프 주기를 2Hz로 설정 (1초에 반복되는 횟수) : 루프 한번이 0.5초에 한번 돌아간다
    ...
    while(ros::ok()) { // ros가 실행이 되면 ok()가 활성화가 된다.
        ...
        loop_rate.sleep();
        count += 2; // 현재 0.5초에 한번 루프가 카운트가 2 올라간다
